        ...
        <v-col class="d-flex align-center border border-primary">
          <v-fade-transition hide-on-leave>
            <v-btn
              ...
              @click="copyUrl"
            >
              <v-icon icon="mdi-content-copy" color="primary" />
          ...
          <v-fade-transition hide-on-leave>
            <v-sheet
              v-if="!editUrl"
              class="mx-2 py-1 px-2 d-flex align-center justify-space-between"
              rounded="xl"
            >
              <v-icon
                v-show="userProfile"
              ...
              <div class="ml-2 d-flex flex-column text-start overflow-hidden">
                <a
                  :href="'/r/' + url.shortened_link"
                  target="_blank"
              ...
              <div class="d-flex align-center">
                <v-tooltip activator="parent" color="accent" location="start">{{
                  url.expired_at
                    ? "Expires " + url.expired_at
                    : "Always available"
                }}</v-tooltip>
          ...
          <v-fade-transition hide-on-leave>
            <div v-if="editUrl" class="w-100 mx-2" rounded="xl">
              <v-text-field
                variant="plain"
                class="bg-white rounded-xl px-4 py-1"
                clearable
                counter
                active
                ...
                persistent-hint
                hint="Unable to edit destination url, visibility and its expiry time"
                color="primary"
                :disabled="newUrlEdited.load"
                :loading="newUrlEdited.load"
          ...
          <div class="d-flex" v-show="userProfile">
            <v-btn
              elevation="0"
              :disabled="newUrlEdited.load || destroy.load"
              @click="editUrl ? handleEditUrl : (editUrl = true)"
              icon
              color="white"
              ><v-icon
                :icon="editUrl ? 'mdi-check-bold' : 'mdi-pencil'"
                color="primary"
              ></v-icon
            ></v-btn>
            <v-spacer></v-spacer>
            <v-btn
              ...
              class="ml-1"
              :loading="destroy.load"
              variant="outlined"
              :icon="editUrl ? 'mdi-close' : 'mdi-delete'"
              color="red"
            ></v-btn>
          </div>
        </v-col>
...
export default defineComponent({
  ...
  methods: {
    copyUrl() {
      try {
        navigator.clipboard.writeText(
          this.site + "/r/" + this.url.shortened_link
        );
    ...
    async handleCheckCustomUrl() {
      if (
        this.url.shortened_link &&
        this.newUrlEdited.shortened_link !== this.url.shortened_link
      ) {
        try {
    ...
    async handleEditUrl() {
      ...
      if ((await this.handleCheckCustomUrl()) && !this.newUrlEdited.error) {
        if (
          this.newUrlEdited.shortened_link != this.$props.url.shortened_link
        ) {
          try {
